...
const BEAST_MEME_TOKEN_MINT = new PublicKey(process.env.TOKEN_ADDRESS); // 🔹 Token mint address from env
const EXCHANGE_RATE = 100000000; // 1 SOL = 100,000,000 BEAST MEME
const MIN_PURCHASE_SOL = 0.1; // 🔹 Minimum 0.1 SOL purchase
const TOKEN_DECIMALS = 9; // 🔹 Adjust based on token decimal places
...
app.post("/pay", async (req, res) => {
    try {
        ...
        const adjustedBeastAmount = beastMemeAmount * (10 ** TOKEN_DECIMALS); // ✅ Adjust for token decimals
        ...
        const transaction = new Transaction().add(
            SystemProgram.transfer({
                ...
                lamports: adjustedBeastAmount, // ✅ Now correctly adjusts for token decimals
            })
        );
